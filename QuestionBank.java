...
public class QuestionBank extends Question{
	...
	private ArrayList<Question> WGquestions = new ArrayList<Question>();
	private ArrayList<String> HWords = new ArrayList<String>();
	
	/**
	* No-argument constructor -- initializes all questions in the question banks and all words in the word bank
	*/
	public QuestionBank(){
		setALLTFQuestions();
		setAllWordGuessQuestions();
		setAllHangmanWords();
	}
	
	/**
	* creates (and shuffles) a Java SE 8 certification prep hangman word bank
	*/
	public void setAllHangmanWords(){
		HWords.add("constructor");
		HWords.add("encapsulation");
		HWords.add("polymorphism");
		HWords.add("NullPointerException");
		HWords.add("static");
		HWords.add("wildcard");
		HWords.add("boolean");
		HWords.add("compiler");
		HWords.add("instance");
		HWords.add("autoboxing");
		HWords.add("ArrayList");
		HWords.add("StringBuilder");
		HWords.add("concatenation");
		HWords.add("varargs");
		HWords.add("LocalDateTime");
		HWords.add("parameter");
		HWords.add("lambdas");
		HWords.add("inheritance");
		HWords.add("interface");
		HWords.add("ArrayIndexOutOfBoundsException");
		HWords.add("extends");
		HWords.add("implements");	
		
		Collections.shuffle(HWords);
	}
	
	/**
	* creates (and shuffles) a Java SE 8 certification prep word guess question bank arraylist
	*/
	public void setAllWordGuessQuestions(){
		Question QWG1 = new Question("This type of variable is defined within a method.", "local");
		WGquestions.add(QWG1);
		Question QWG2 = new Question("This operator reverses/flips the value of a boolean.", "logical complement");
		WGquestions.add(QWG2);
		Question QWG3 = new Question("This is what we call compiled java code (code with a .class extension).", "bytecode");
		WGquestions.add(QWG3);
		Question QWG4 = new Question("Where a Java program begins execution.", "main method");
		WGquestions.add(QWG4);
		Question QWG5 = new Question("This operator compares two String objects to see if they reference the same memory location.", "==");
		WGquestions.add(QWG5);
		Question QWG6 = new Question("This String method removes whitespace from the beginning and end of a String.", "trim");
		WGquestions.add(QWG6);
		//ternary expressions
		//float and double
		//StringBuilder
		//exception handling
		...
		//static keyword
		//lambda expression
		Collections.shuffle(WGquestions);
	}
	
	/**
	* creates (and shuffles) a Java SE 8 certification prep true/false question bank arraylist
	*/
	public void setALLTFQuestions(){
		...
		Question Q7 = new Question("The abstract specifier is used when a method is not " + 
			"allowed to be overridden by a subclass.", false, "The final specifier is used " + 
	...
	/**
	* returns the Java SE 8 certification prep word guess question bank
	* @return ArrayList the question bank
	*/
	public ArrayList<Question> getAllWordGuessQuestions(){
		return WGquestions;	
	}
	
	/**
	* returns the Java SE 8 certification prep true/false question bank
	* @return ArrayList the question bank
	*/
	public ArrayList<Question> getALLTFQuestions(){
		return questions;	
	}
	
	/**
	* returns the Java SE 8 certification prep hangman word bank
	* @return ArrayList the word bank
	*/
	public ArrayList<String> getAllHWords(){
		return HWords;	
	}
}